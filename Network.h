...
private:
  ...
  std::list<NodePtr> get_nodes_from_level(
      int type,
      int level,
      bool match_type);
...
public:
  ...
  // Adds a node of specified id of a type at desired level. Default level is 0.
  NodePtr add_node(
      string id,
      int type,
      int level = 0);
  ...
  NodePtr create_group_node(
      int type,
      int level);


  // Grabs and returns node of specified id, at desired level. Default level is 0
  NodePtr get_node_by_id(
      string id,
      int level = 0);
